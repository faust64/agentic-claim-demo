...
- ⚠️ MCP tool calls fail with `Field required: input` error (SDK bug)
- ⚠️ Vector store insertion only keeps last batch (overwrites previous data)

**Recent MCP Server Improvements (applied to both branches):**
...
- ⚠️ **Critical**: LlamaStack 0.3.0rc3+rhai0 has a bug preventing MCP tool execution
  ...
  - MCP tools are discovered but never called by the Agent/Responses API
